...
const SELECTORS = {
  ...
  // 新版：优先用 aria-label，兼容中英文
  newThreadButton:
    'button[aria-label="新建主题"], button[aria-label="New thread"], button[aria-label*="新建"]',
...
class PerplexityApp {
  ...
  async start({ text, mode }) {
    ...
    let inputField =
      document.querySelector(SELECTORS.followUpInput) ||
  ...
  async observeMutations() {
    ...
    this.observer = new MutationObserver(async () => {
      ...
      if (
        isComplete &&
        responseData.text &&
        responseData.text !== this.lastText
      ) {
        ...
        if (this.socket && this.socket.readyState === WebSocket.OPEN) {
          log(
            "Sending complete response with",
            responseData.citations.length,
            "citations",
          );
          this.socket.send(
            JSON.stringify({
